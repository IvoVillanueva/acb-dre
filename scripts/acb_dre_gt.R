...
df <- jornada_dre %>%
  ...
  dplyr::filter(
    num_jornada == max(num_jornada) &
      !is.na(license_license_str15)
  ) %>%
  dplyr::mutate(
  ...
  dplyr::select(
    ...
    matches, time_played, ts_pct, usg_pct, points,
    total_rebound, asis, turnovers, steals, blocks,
    x2pm_a, x3pm_a, x1pm_a, differential, val, dre
  ) %>%
  dplyr::arrange(dplyr::desc(dre)) %>%
  dplyr::left_join(racha, dplyr::join_by(abb)) %>%
  dplyr::left_join(clubs, dplyr::join_by(abb)) %>%
  dplyr::mutate(
    rival = dplyr::case_when(
      grepl("\\(W", rival) ~ gsub(
        "\\(W",
        "<span style='color:#1a7f37;'>(W", rival
      ),
      grepl("\\(L", rival) ~ gsub(
        "\\(L",
        "<span style='color:#d32f2f;'>(L", rival
      ),
      TRUE ~ rival
    ),

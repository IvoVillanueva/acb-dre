# --- 1. CARGA DE LIBRERÍAS (Asegura que los helpers puedan usarlas) ---
library(tidyverse)
library(janitor)
library(glue)
library(htmltools)
library(lubridate)
library(reactable) # Aunque no se usa directamente aquí, es buena práctica tenerla

# --- 2. CARGA DE HELPERS Y DATOS ---
# Este script carga 'racha', 'jornada_dre', 'clubs', 'totales_equipo', etc.
source("scripts/helpers_table.R")

# --- 3. PIPELINE DE PROCESAMIENTO ---

df <- jornada_dre %>%
  janitor::clean_names() %>%
  dplyr::left_join(totales_equipo, by = c("id_match", "abb")) %>%
  ...
  dplyr::mutate(
    ...
    time_played = sprintf(
      "%02d:%02d",
      lubridate::minute(time_played),
      lubridate::second(time_played)
    ),
    fecha = format(fecha, "%d/%m/%Y"),
    # Formato de tiros
    x2pm_a = paste0(x2pt_success, "/", x2pt_tried),
    x3pm_a = paste0(x3pt_success, "/", x3pt_tried),
    x1pm_a = paste0(x1pt_success, "/", x1pt_tried), # Corregido: Usaba x3pt_success antes
  ...
  dplyr::left_join(racha, dplyr::join_by(abb)) %>%
  dplyr::left_join(clubs, dplyr::join_by(abb)) %>%
  dplyr::mutate(
    # Aplicar color al W/L streak del rival
    rival = case_when(
      grepl("\\(W", rival) ~ gsub(
        "\\(W",
    ...
    # Formato de porcentajes finales

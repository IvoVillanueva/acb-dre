  ...
  columns = list(
   ...
   dre = colDef(
      ...
      style = function(value, index) {
        # --- FIX PARA EL ERROR 'rgb() NA' ---
        
        # 1. Si el valor es NA, devolvemos fondo blanco y paramos aquí.
        if (is.na(value)) {
          return(list(background = "#ffffff", color = "#000000"))
        }

        # 2. Calculamos min y max ignorando NAs (na.rm = TRUE)
        min_dre <- min(df$dre, na.rm = TRUE)
        max_dre <- max(df$dre, na.rm = TRUE)
        
        # Protección extra: Si min == max (todos iguales), evitamos división por cero
        if (max_dre == min_dre) {
           return(list(background = "#ffffff", color = "#000000"))
        }
